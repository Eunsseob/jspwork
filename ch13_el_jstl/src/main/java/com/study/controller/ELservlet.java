...
public class ELservlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 		요청 때마다 매번 생성되는 객체
		 * 		이 영역에 데이터를 담으면 해당 request객체를 포워딩 받는 응답 jsp 또는 sevlet에서만 사용가능
		 * 		공유범위가 작음
		...
		request.setAttribute("student", new Person("더조은", 12, "남자"));
		
		// sessionScope에 담기
		HttpSession session = request.getSession();
		session.setAttribute("academy", "tjouen");
		session.setAttribute("teacher", new Person("손은섭", 27, "남자"));
